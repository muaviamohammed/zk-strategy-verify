# Gate-Policy Specification — v0.3.1 (draft)
...
| `fee_bps` / `slippage_bps` | Committed cost model (bps per position change) applied inside the proof | If the policy requires `cost_floor`: MUST meet the protocol floor (§3.9). Absent → legacy default = the floor |
...
9. **cost_floor** — the committed cost model meets the protocol floor: **fee ≥ 7 bps and slippage ≥ 3 bps** per position change. Understated costs are a forge vector — they resurrect dead edges — so a conforming verifier refuses sub-floor costs outright. Costs may always be committed *above* the floor ("prove at your real costs"); proving at harsher costs strictly strengthens the credential. Fee relaxation below the floor exists only **out-of-band**, against an issuer-attested institutional fee schedule explicitly accepted by the relying party; the slippage floor is **never waivable** — fill quality cannot be documented, only proven from live fills. The floor is a *minimum standard*, not an endorsement: for thin instruments even the floor is generous, and the committed number is always on the face of the credential.
